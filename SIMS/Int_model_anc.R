...
initialize() {
    initializeMutationType("m1", 0.5, "f", 0); // Mutation in ANC is neutral
...
origin_pop <- "ANC"
target_pop <- "NEA"
onset_time <- runif(1, min = 100e3, max = 650e3) # Time arising the mutation
...
gf <- list(gene_flow(from = eur, to = E_afr, start = 13e3, end = 12e3, rate = 0.12), # BACK TO AFRICA
           ...
           gene_flow(from = eur, to = chb, start = 36e3, end = 0, rate = 1-((1-3.14e-5)^(36e3/29))), # Stpopsim Pap 1-((1-31.4e-4)^(36e3/(29*Q)))
           gene_flow(from = chb, to = eur, start = 36e3, end = 0, rate = 1-((1-3.14e-5)^(36e3/29))), # Stpopsim Pap (31.4e-4)/2
           gene_flow(from = ANC, to = nea, start = 200e3, end = 199e3, rate = 0.05), # li et al
           gene_flow(from = ANC, to = nea, start = 100e3, end = 99e3, rate = 0.005))
...
U_CHB_DEN <- extract.U(Fr_EAF,Fr_CHB,Fr_DEN,0.01,0.2) # U.1.20.100
U_EUR_DEN <- extract.U(Fr_EAF,Fr_EUR,Fr_DEN,0.01,0.2) # U.1.20.100
DfD_CHB_NEA <- getD_stats(Fr_EAF,Fr_CHB,Fr_NEA)
DfD_EUR_NEA <- getD_stats(Fr_EAF,Fr_EUR,Fr_NEA)
U_CHB_NEA <- extract.U(Fr_EAF,Fr_CHB,Fr_NEA,0.01,0.2) # U.1.20.100
U_EUR_NEA <- extract.U(Fr_EAF,Fr_EUR,Fr_NEA,0.01,0.2) # U.1.20.100
...
pos_init <- toString(extra_chunk)
pos_finit <- toString(simulated_chunk-extra_chunk)
...
parameter.df <- data.frame(s_den/Q,s_nea/Q,s_chb/Q,s_eur/Q,onset_time,replicate,mutation,time_simulation)
