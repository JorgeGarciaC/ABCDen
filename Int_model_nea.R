...
initialize() {
    initializeMutationType("m1", 0.5, "f", s_nea); // Mutation in ANC is neutral
...
origin_pop <- "NEA"
target_pop <- "OOA"
onset_time <- runif(1, min = 100e3, max = 600e3) # Time arising the mutation
...
gf <- list(gene_flow(from = eur, to = E_afr, start = 13e3, end = 12e3, rate = 0.12), # BACK TO AFRICA
           ...
           gene_flow(from = eur, to = chb, start = 36e3, end = 0, rate = 1-((1-3.14e-5)^(36e3/29))), 
           gene_flow(from = chb, to = eur, start = 36e3, end = 0, rate = 1-((1-3.14e-5)^(36e3/29))), 
